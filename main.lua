...
tween = require("loveframes/tween")

win = {}
text = "Click the button to generate a name"

win.width, win.height, win.flags = love.window.getMode()
function love.load()         
	love.graphics.setBackgroundColor(1,1,1,1) --White
	love.graphics.setColor(0,0,0,1) --Black
	font = love.graphics.newFont("assets/font.ttf", 58)

	newButton = loveframes.Create("button")
	newButton:SetText("NEW")
	newButton:SetWidth(win.width - 5)
	newButton:SetHeight(win.height/15)
	newButton:SetPos(0,win.height-newButton:GetHeight())
	newButton:CenterX()
	newButton.OnClick = function(object, x, y)
		newItem()
	end

	--local scrollbar = loveframes.Create("slider")
	--scrollbar:SetHeight(win.height - 10 - newButton:GetHeight())
	--scrollbar:SetWidth(25)
	--scrollbar:SetSlideType("vertical")
	--scrollbar:SetPos(win.width - 15, 0)
end

function love.draw()
	--love.graphics.print("Category", win.width/2, 15)
	love.graphics.printf(text, font, 0, win.height/2, win.width, "center")
	loveframes.draw()
end
